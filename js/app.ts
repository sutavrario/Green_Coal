...
const App = (() => {
  ...
  function createBackgroundParticles() {
    const container = document.getElementById('bgParticles') as any;
    if (!container) return;
  ...
  function setupMobileMenu() {
    const toggle = document.getElementById('menuToggle') as any;
  ...
  function loadPage(page) {
    ...
    const activePage = document.getElementById(`page-${page}`) as any;
    if (activePage) {
    ...
    switch (page) {
      ...
      case 'ai-insights': initAIInsights(); break;
      case 'leaderboard': initLeaderboard(); break;
      case 'process': initProcess(); break;
    }
  }
  ...
  function renderConversionChart() {
    const ctx = document.getElementById('conversionChart') as any;
    if (!ctx) return;
  ...
  function renderCompositionChart() {
    const ctx = document.getElementById('compositionChart') as any;
    if (!ctx) return;
  ...
  function renderActivityFeed() {
    const container = document.getElementById('activityFeed') as any;
    if (!container) return;
  ...
  function renderSystemStatus() {
    const container = document.getElementById('systemStatus') as any;
    if (!container) return;
  ...
  function updateMonitorTime() {
    const el = document.getElementById('monitorTime') as any;
    if (el) {
  ...
  function updateGauge(id, value, min, max, unit, label) {
    ...
    if (fillEl) {
      (fillEl as HTMLElement).style.strokeDasharray = `${arcLength}`;
      (fillEl as HTMLElement).style.strokeDashoffset = `${arcLength * (1 - pct)}`;
    }
    ...
    if (id === 'tempGauge') {
      if (value < 200 || value > 300) { statusText = 'Critical'; statusClass = 'critical'; }
      else if (value < 210 || value > 290) { statusText = 'Warning'; statusClass = 'warning'; }
    } else if (id === 'gasGauge') {
      if (value > 40) { statusText = 'Critical'; statusClass = 'critical'; }
      else if (value > 25) { statusText = 'Warning'; statusClass = 'warning'; }
    } else if (id === 'moistureGauge') {
      if (value > 25) { statusText = 'Critical'; statusClass = 'critical'; }
      else if (value > 20) { statusText = 'Warning'; statusClass = 'warning'; }
    } else if (id === 'efficiencyGauge') {
      if (value < 65) { statusText = 'Critical'; statusClass = 'critical'; }
      else if (value < 75) { statusText = 'Warning'; statusClass = 'warning'; }
    }
  ...
  function renderSensorHistoryCharts() {
    ...
    // Temperature History
    const tempCtx = document.getElementById('tempHistoryChart') as any;
    if (tempCtx) {
    ...
    const gasCtx = document.getElementById('gasHistoryChart') as any;
    if (gasCtx) {
      if (charts.gasHistory) charts.gasHistory.destroy();
  ...
  function renderAlerts() {
    const container = document.getElementById('alertsList') as any;
    if (!container) return;
  ...
  function renderClassification() {
    const container = document.getElementById('classificationResults') as any;
    if (!container) return;
  ...
  function renderForecastChart() {
    const ctx = document.getElementById('forecastChart') as any;
    if (!ctx) return;
  ...
  function renderRecommendations() {
    const container = document.getElementById('recommendations') as any;
    if (!container) return;
  ...
  function renderAnomalies() {
    const container = document.getElementById('anomalyList') as any;
    if (!container) return;
  ...
  function setupLeaderboardFilters() {
    ...
    document.querySelectorAll('.filter-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        currentPeriod = (e.target as HTMLElement).dataset.period;
        renderLeaderboard();
      });
    });

    // Zone filter
    const zoneFilter = document.getElementById('zoneFilter') as any;
    if (zoneFilter) {
      zoneFilter.addEventListener('change', (e) => {
        currentZone = (e.target as HTMLInputElement).value;
        renderLeaderboard();
      });
    }
  }
  ...
  function getFilteredLeaderboard() {
    let data = MockData.getLeaderboardData().map(ward => {
      ...
      switch (currentPeriod) {
        case 'weekly': points = ward.weeklyPoints; break;
        case 'monthly': points = ward.monthlyPoints; break;
  ...
  function renderTotalStats(data) {
    const container = document.getElementById('lbTotalStats') as any;
    if (!container) return;
  ...
  function renderPodium(data) {
    ...
    if (!container || data.length < 3) {
      if (container) container.innerHTML = '<p style="color: var(--text-muted); text-align: center;">Not enough data for podium</p>';
      return;
    }
  ...
  function renderRankingTable(data) {
    const tbody = document.getElementById('rankingTableBody') as any;
    if (!tbody) return;
  ...
  function toggleWardDetails(wardId) {
    const detailsRow = document.getElementById(`wardDetails-${wardId}`) as any;
  ...
  function initProcess() {
    ...
    steps.forEach((step, i) => {
      (step as HTMLElement).style.animationDelay = `${i * 0.15}s`;
    });
  }
