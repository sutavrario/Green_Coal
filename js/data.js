...
const MockData = (() => {
    ...
    function getSensorReading() {
        ...
        sensorState.temperature = Math.max(195, Math.min(305, sensorState.temperature + randomFloat(-3, 3)));
        sensorState.gasEmission = Math.max(2, Math.min(50, sensorState.gasEmission + randomFloat(-1.5, 1.5)));
        sensorState.moisture = Math.max(3, Math.min(30, sensorState.moisture + randomFloat(-1, 1)));
        sensorState.efficiency = Math.max(60, Math.min(98, sensorState.efficiency + randomFloat(-2, 2)));
        return {
