...
const PublicApp = (() => {
    ...
    function renderNotices() {
        ...
        const tickerWrap = document.getElementById('urgentTicker');
        if (!container)
            return;
    ...
    function renderHeroStats() {
        ...
        if (els.totalWaste)
            els.totalWaste.textContent = stats.totalWasteCollected.toLocaleString('en-IN');
        if (els.totalCoal)
            els.totalCoal.textContent = stats.totalWasteConverted.toLocaleString('en-IN');
        if (els.totalCarbon)
            els.totalCarbon.textContent = stats.totalCarbonSaved.toLocaleString('en-IN');
        if (els.totalUsers)
            els.totalUsers.textContent = stats.totalUsers.toLocaleString('en-IN');
    }
    // ── Ward Selector ─────────────────────
    ...
    function renderWardInfo() {
        ...
        if (!ward)
            return;
        const wiWaste = document.getElementById('wiWaste');
        const wiCoal = document.getElementById('wiCoal');
        const wiCarbon = document.getElementById('wiCarbon');
        const wiPoints = document.getElementById('wiPoints');
        const wiUsers = document.getElementById('wiUsers');
        const wiRank = document.getElementById('wiRank');
        const wiRankBadge = document.getElementById('wiRankBadge');
        if (wiWaste)
            wiWaste.textContent = `${ward.wasteCollected.toLocaleString('en-IN')} kg`;
        if (wiCoal)
            wiCoal.textContent = `${ward.wasteConverted.toLocaleString('en-IN')} kg`;
        if (wiCarbon)
            wiCarbon.textContent = `${ward.carbonSaved.toLocaleString('en-IN')} kg`;
        if (wiPoints)
            wiPoints.textContent = ward.points.toLocaleString('en-IN');
        if (wiUsers)
            wiUsers.textContent = ward.activeUsers;
        // Rank
        const lb = Store.getLeaderboard('points');
        const rank = lb.findIndex(w => w.id === selectedWardId) + 1;
        if (wiRank)
            wiRank.textContent = `#${rank}`;
        if (wiRankBadge) {
    ...
    function renderNextCollection() {
        ...
        const ncDetail = document.getElementById('ncDetail');
        const ncStatus = document.getElementById('ncStatus');
        const card = document.getElementById('nextCollectionCard');
        if (ncTime)
            ncTime.textContent = `${mySched.day} • ${mySched.time}`;
        if (ncDetail)
            ncDetail.textContent = `Collector Unit: ${mySched.collector} | Zone: ${mySched.zone}`;
        if (ncStatus) {
    ...
    function renderProcessStatus() {
        ...
        if (el.label)
            el.label.textContent = proc.stageLabel || 'Processing...';
        if (el.batch)
            el.batch.textContent = `Batch #${proc.currentBatch || '—'}`;
        if (el.fill)
            el.fill.style.width = `${proc.progress || 0}%`;
        if (el.pct)
            el.pct.textContent = `${proc.progress || 0}%`;
        if (el.eta)
            el.eta.textContent = proc.eta || '—';
        if (el.total)
            el.total.textContent = proc.totalBatchesToday || '—';
        if (el.completed)
            el.completed.textContent = proc.completedBatches || '—';
        if (el.sysBadge) {
        ...
        document.querySelectorAll('.stage-step').forEach(step => {
            ...
            if (step.dataset.stage === proc.stage)
    ...
    function renderLeaderboard() {
        const container = document.getElementById('lbBody');
        if (!container)
            return;
        const wards = Store.getLeaderboard(currentSort);
    ...
    function renderImpact() {
        ...
        if (el.waste)
            el.waste.textContent = `${stats.totalWasteCollected.toLocaleString('en-IN')} kg`;
        if (el.coal)
            el.coal.textContent = `${stats.totalWasteConverted.toLocaleString('en-IN')} kg`;
        if (el.carbon)
            el.carbon.textContent = `${stats.totalCarbonSaved.toLocaleString('en-IN')} kg`;
    }
    // ── Update Timestamp ──────────────────
    function updateTimestamp() {
        const el = document.getElementById('lastUpdated');
        if (el)
            el.textContent = `Last synced: ${new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}`;
    }
    // ── Utilities ─────────────────────────
