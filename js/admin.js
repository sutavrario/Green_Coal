...
const Admin = (() => {
    ...
    function triggerCurtain(callback) { callback(); }
    ...
    function loadSection(section) {
        ...
        document.querySelectorAll('.admin-section').forEach(s => { s.classList.remove('active'); void s.offsetWidth; });
        ...
        if (ti) {
            document.getElementById('secTitle').textContent = ti.t;
            document.getElementById('secSubtitle').textContent = ti.s;
        }
    ...
    function renderWardData() {
        ...
        wardSelect.onchange = () => {
            ...
            if (!wid) {
                document.getElementById('wardForm').style.display = 'none';
                return;
            }
            ...
            document.getElementById('wdWaste').value = ward.wasteCollected;
            document.getElementById('wdCoal').value = ward.wasteConverted;
            document.getElementById('wdCarbon').value = ward.carbonSaved;
            document.getElementById('wdPoints').value = ward.points;
            document.getElementById('wdUsers').value = ward.activeUsers;
        };
        // Save handler
        document.getElementById('wardSaveBtn').onclick = () => {
            const wid = parseInt(wardSelect.value);
            if (!wid)
                return toast('Select a ward first', 'error');
            Store.updateWard(wid, {
                wasteCollected: parseFloat(document.getElementById('wdWaste').value) || 0,
                wasteConverted: parseFloat(document.getElementById('wdCoal').value) || 0,
                carbonSaved: parseFloat(document.getElementById('wdCarbon').value) || 0,
                points: parseInt(document.getElementById('wdPoints').value) || 0,
                activeUsers: parseInt(document.getElementById('wdUsers').value) || 0,
            });
    ...
    function renderSchedules() {
        ...
        const container = document.getElementById('schedulesBody');
    ...
    function saveSchedule(wardId) {
        const day = document.querySelector(`.sched-day[data-ward="${wardId}"]`).value;
        const time = document.querySelector(`.sched-time[data-ward="${wardId}"]`).value;
        const status = document.querySelector(`.sched-status[data-ward="${wardId}"]`).value;
        const collector = document.querySelector(`.sched-unit[data-ward="${wardId}"]`).value;
    ...
    function renderNotices() {
        ...
        const previewArea = document.getElementById('noticePreview');
        // Type selector
        document.querySelectorAll('.notice-type-btn').forEach(btn => {
            btn.classList.toggle('active', btn.dataset.type === selectedNoticeType);
            btn.onclick = () => {
                selectedNoticeType = btn.dataset.type;
      ...
      `;
        ...
        document.getElementById('postNoticeBtn').onclick = () => {
            ...
            if (!title || !message)
                return toast('Fill in both title and message', 'error');
    ...
    function renderIoT() {
        const iot = Store.getIoT();
        // Set slider values
        const tempSlider = document.getElementById('iotTemp');
        const gasSlider = document.getElementById('iotGas');
        const moistSlider = document.getElementById('iotMoist');
        const effSlider = document.getElementById('iotEff');
        if (tempSlider) {
            tempSlider.value = iot.temperature;
        ...
        ['iotTemp', 'iotGas', 'iotMoist', 'iotEff'].forEach(id => {
            ...
            if (el)
                el.oninput = () => { document.getElementById(id + 'Val').textContent = parseFloat(el.value).toFixed(1); };
        });
        // System toggle
        const sysToggle = document.getElementById('sysOnlineToggle');
        if (sysToggle)
            sysToggle.checked = iot.systemOnline;
        // Reactor status
        const reactorSel = document.getElementById('reactorStatus');
        if (reactorSel)
            reactorSel.value = iot.reactorStatus || 'running';
        // Save
    ...
    function renderProcess() {
        ...
        document.getElementById('procBatch').value = proc.currentBatch || '';
        document.getElementById('procStage').value = proc.stage || 'torrefaction';
        document.getElementById('procProgress').value = proc.progress || 50;
        document.getElementById('procProgressVal').textContent = `${proc.progress || 50}%`;
        document.getElementById('procEta').value = proc.eta || '';
        document.getElementById('procTotalBatches').value = proc.totalBatchesToday || '';
        document.getElementById('procCompletedBatches').value = proc.completedBatches || '';
        document.getElementById('procProgress').oninput = (e) => {
            document.getElementById('procProgressVal').textContent = `${e.target.value}%`;
        };
        document.getElementById('procSaveBtn').onclick = () => {
            ...
            const stage = document.getElementById('procStage').value;
            Store.updateProcess({
                currentBatch: parseInt(document.getElementById('procBatch').value) || 0,
                stage,
                stageLabel: stageLabels[stage] || stage,
                progress: parseInt(document.getElementById('procProgress').value) || 0,
                eta: document.getElementById('procEta').value,
                totalBatchesToday: parseInt(document.getElementById('procTotalBatches').value) || 0,
                completedBatches: parseInt(document.getElementById('procCompletedBatches').value) || 0,
            });
